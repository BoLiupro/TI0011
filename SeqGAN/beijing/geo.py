...
def avg_user_movement_m(coords, sequences, user_ids):
    """Average, over users, of each user's mean haversine displacement (meters) between
    chronologically-consecutive real hours, counting only steps where the user actually
    moved to a different region (zero-displacement 'stayed put' steps are dropped before
    averaging). Users who never move at all (no non-zero step in any of their sequences)
    are excluded entirely, rather than pulling the average down with an empty/zero mean.
    """
    from collections import defaultdict as _defaultdict
    ...
    for seq, uid in zip(sequences, user_ids):
        ...
        lon = coords[seq, 1]
        d = haversine_np(lat[:-1], lon[:-1], lat[1:], lon[1:])
        user_to_dists[uid].extend(d[d > 0].tolist())
    ...
    for uid, dists in user_to_dists.items():
        ...
        if arr.size == 0:
            skipped += 1
            continue
        per_user_means.append(arr.mean())

    if skipped:
        print('avg_user_movement_m: excluded %d/%d users who never moved' %
              (skipped, len(user_to_dists)))
    return float(np.mean(per_user_means))
