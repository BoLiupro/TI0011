...
def generate_pool(generator, num, batch_size, device):
    """Unconditioned generation of `num` sequences, in batches."""
    out = []
    remaining = num
    while remaining > 0:
        b = min(batch_size, remaining)
        samples = generator.generate(batch_size=b, device=device, sample=True)
        out.append(samples)
        remaining -= b
...
def adversarial_generator_step(generator, discriminator, opt, batch_size, device):
    samples = generator.generate(batch_size=batch_size, device=device, sample=True)
    rewards = rollout.get_reward(generator, discriminator, samples, rollout_num=config.ADV_ROLLOUT_NUM)
    log_probs = generator.sampled_log_probs(samples)
...
def main():
    ...
    for rnd in range(args.d_pretrain_rounds):
        generator.eval()
        fake_pool = generate_pool(generator, args.generated_num, config.BATCH_SIZE, device)
        generator.train()
